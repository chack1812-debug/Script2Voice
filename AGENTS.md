# Script2Voice-Rust版 — Codex project-local rules
...
`.codex/config.toml`ではhooksが有効であり、`.codex/hooks.json`はSessionStart、UserPromptSubmit、PreCompact、PostCompactで`bd codex-hook`を呼ぶ。hookはcontext refreshの補助であり、権限・承認・Source of Truthを置き換えない。hookが安全または非変更と仮定せず、手動実行・設定変更は明示的な依頼がある場合だけ行う。
